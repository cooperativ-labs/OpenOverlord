...
export type ObjectiveState = 'future' | 'draft' | 'submitted' | 'launching' | 'executing' | 'pending_delivery' | 'complete';
...
export interface ProjectDto {
    ...
    /**
     * Name of the Everhour project this Overlord project is linked to (stored in
     * project settings). `null` means not yet linked; the UI defaults the input to
     * the Overlord project name. Only meaningful when a workspace Everhour API key
     * is configured.
     */
    everhourProjectName: string | null;
    /**
     * Everhour project id (e.g. `ev:1234567890`) resolved when the project is
     * linked. `null` until linking succeeds. Read-only from the client's view.
     */
...
export type TargetObservationState = 'available' | 'missing' | 'unreachable' | 'permission_denied' | 'not_git_repository' | 'unknown';
...
export type ProjectRepositoryStatus = 'ready' | 'no_resource' | 'not_git_repository' | 'unsupported_resource' | 'unreadable';
...
export type ArtifactType = 'test_results' | 'next_steps' | 'note' | 'url' | 'decision' | 'migration';
 ...
 */
export type MissionEventType = 'update' | 'user_follow_up' | 'alert' | 'discussion_summary' | 'decision' | 'ask' | 'permission_request' | 'delivery' | 'execution_requested' | 'awaiting_approval' | 'status_change';
...
export type ExecutionRequestStatus = 'queued' | 'claimed' | 'launching' | 'launched' | 'failed' | 'cleared' | 'cancelled' | 'expired';
...
/** Body for `PUT /api/integrations/everhour` — sets/replaces the workspace API key. */
export interface UpdateEverhourIntegrationBody {
    apiKey: string;
}
/**
 * Body for `PUT /api/projects/:id/everhour-link` — links the Overlord project to
 * an Everhour project by name. A non-empty name finds-or-creates the matching
 * Everhour project and stores its id + name; `null`/empty unlinks. Returns the
 * refreshed `ProjectDto`.
 */
...
/** Body for `POST /api/missions/:id/everhour/time` — add a manual time record. */
...
/** Body for `PATCH /api/missions/:id/everhour/time/:recordId` — edit a time record. */
