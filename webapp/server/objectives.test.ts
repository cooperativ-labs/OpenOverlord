...
const { createTicket, createProject, createObjective, updateObjective } =
...
test('new draft objectives inherit the project last-used agent', () => {
  ...
  // The ticket's first objective is created through the same insert path and must
  // record the launch selection so the button and execution read it from the db.
  const ticket = createTicket({ projectId: project.id, firstObjective: 'Do the thing' });
  const firstObjective = ticket.objectives[0]!;
  ...
  const added = createObjective({ ticketId: ticket.id, instructionText: '' });
...
test('new draft objectives leave the agent unset without a launch preference', () => {
  ...
  const ticket = createTicket({ projectId: project.id, firstObjective: 'Do the thing' });
  assert.equal(ticket.objectives[0]!.assignedAgent, null);
});
