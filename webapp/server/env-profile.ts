...
// This module ships at `webapp/server` in the source tree and is bundled
// elsewhere in production, so its own location distinguishes the two layouts.
const here = path.dirname(fileURLToPath(import.meta.url));

/** Repo root inferred from this module's source location (`webapp/server` → up two). */
export const REPO_ROOT = path.resolve(here, '..', '..');

/**
 * Source-vs-bundled env profile, shared by every server entry module so the
 * detection lives in one place. The source server runs from `webapp/server` and
...
export const ENV_PROFILE: EnvProfile =
  here === path.join(REPO_ROOT, 'webapp', 'server') ? 'development' : 'production';
