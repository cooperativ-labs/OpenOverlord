// Bundle the Overlord backend (`backend/index.ts`) and (almost) its entire
...
// forks inside an Electron `utilityProcess`. The dev flow keeps using
// `tsx backend/index.ts` unchanged.
//
...
const outdir = path.join(backendRoot, 'dist-server');
const cloudBuild =
...
await build({
  ...
  banner: {
    js: [
      '// Overlord backend bundle - generated by backend/scripts/build-server.mjs. Do not edit.',
...
const postgresMigrationsDest = path.join(backendRoot, 'postgres', 'migrations');
rmSync(path.join(backendRoot, 'postgres'), { recursive: true, force: true });
...
if (cloudBuild) {
  rmSync(path.join(backendRoot, 'sqlite'), { recursive: true, force: true });
  console.log('Built backend/dist-server/index.cjs (cloud: postgres migrations only)');
} else {
  const sqliteMigrationsDest = path.join(backendRoot, 'sqlite', 'migrations');
  rmSync(path.join(backendRoot, 'sqlite'), { recursive: true, force: true });
  ...
  console.log(
    'Built backend/dist-server/index.cjs (+ staged sqlite/migrations and postgres/migrations)'
  );
}
