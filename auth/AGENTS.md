...
3. Check `src/rbac/` for existing authorization code patterns.
...
src/rbac/        ← RBAC authorization logic (colocated with tests)
  authorizer.ts
  authorizer.test.ts
  roles.ts
  permissions.ts
...
New auth provider implementations go under `auth/providers/<name>/` or `src/auth/<name>/` — keep the pattern consistent with whatever `src/rbac/` establishes.
