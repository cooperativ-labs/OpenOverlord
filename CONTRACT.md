...
- Configuration file locations and formats (`overlord.toml`, `.overlord/project.json`), resolved by walking up from the working directory with a documented packaged-mode fallback to the OS app-data directory (e.g. `~/Library/Application Support/Overlord/overlord.toml`), including backend URL settings (`backend_url` for the active REST/backend endpoint, `backend_mode` as `local` or `cloud`), optional legacy database settings consumed only by local/backend packages. Terminal launch settings live on `user_execution_target_preferences.terminal_profile_json` for the local execution target fingerprint (provisioned from the device fingerprint during `ovld setup`). Config resolution layers, highest precedence first: an explicit runtime env var set before any env file loads, the resolved `overlord.toml`, a profile-specific env-file baked-in default (`.env.local` for development, `.env.prod` for production/package builds) next to the resolved toml, then a hardcoded fallback — see `cli/src/env.ts`. Each `ovld` process resolves its own config independently from its own working directory/env; the shared/committed `overlord.toml` must therefore stay host-context-neutral (the loopback default), with any per-process or per-environment override (e.g. a Docker-internal host alias) supplied as a runtime env var at launch instead of edited into the shared file.
