...
- Desktop OAuth handoff endpoints: authenticated `GET /api/auth/desktop/callback` mints an opaque, short-lived, single-use handoff ticket from the browser session and redirects only to `overlord://auth/callback`; unauthenticated `POST /api/auth/desktop/exchange` consumes that ticket once and returns the Better Auth session token to the desktop shell's main process. Neither endpoint returns GitHub credentials, and the ticket itself is not an API credential
...
- **Remote OAuth handoff**: Social OAuth opens in the system browser. After Better Auth establishes its browser session, `/api/auth/desktop/callback` redirects only to `overlord://auth/callback?ticket=…`; the ticket is opaque, single-use, and short-lived. Electron's main process exchanges it over HTTPS at `/api/auth/desktop/exchange`, writes the session token to the active profile's `safeStorage`, and reloads the local shell. The renderer and custom URL never receive a session token, GitHub token, or client secret
