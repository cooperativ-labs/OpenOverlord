...
//
// Time tracking via Everhour (https://api.everhour.com). The workspace API key is
// stored server-side (`workspaces.settings_json`) and never returned to the
// client; the client only learns whether a key is present so it can gate the
// Everhour UI. All Everhour API calls are proxied through the REST API Layer so
// the key never reaches the browser. Durations are in seconds and dates are
// `YYYY-MM-DD`, matching the Everhour API.
