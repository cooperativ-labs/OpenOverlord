...
```bash
pi --model <provider/id> --thinking <level> @<context-file> "Attach, then immediately execute <mission>'s current objective."
```
...
- The visible message must explicitly direct execution without waiting for confirmation.
