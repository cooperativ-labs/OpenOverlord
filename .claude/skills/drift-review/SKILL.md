...
When a change touches agent launch commands, also compare the human-launch surfaces: `packages/open-overlord/bin/_cli/launcher.mjs`, `lib/overlord/launch-commands.ts`, and any ticket copy surfaces that emit copy/paste launch commands.
...
| **CLI commands** | `packages/open-overlord/bin/_cli/protocol.mjs` | `ovld protocol <subcommand>`, `--kebab-case` flags |
...
#### 2. CLI Commands
Read `packages/open-overlord/bin/_cli/protocol.mjs`. Extract:
...
If the task changes human launch commands, also read `packages/open-overlord/bin/_cli/launcher.mjs` and `lib/overlord/launch-commands.ts` so you can compare `ovld launch` / `ovld restart` help, aliases, and emitted copy commands against Desktop and product copy.
...
When auditing Antigravity-only drift, also read the installed-path patch logic in `packages/open-overlord/bin/_cli/setup.mjs` (`patchAntigravityInstalledPaths`) so MCP `args` and hook `command` paths match the staged runtime scripts.
...
#### 6. CLI README
Read `packages/open-overlord/README.md`.
...
- Operations not documented (or inaccurately documented) in `packages/open-overlord/README.md`
...
- `packages/open-overlord/README.md` command or flag examples that no longer match `packages/open-overlord/bin/_cli/protocol.mjs`
