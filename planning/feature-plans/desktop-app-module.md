...
- **Packaging (`electron-builder.yml`).** Bundles `packages/overlord-cli/**`
...
- The desktop module **depends on** `webapp` (URL + REST surface) and the
  packaged `overlord-cli`, but `webapp` and `cli` must have **no build/runtime
...
- **Bundle the CLI + connector plugins** so the spawned `ovld` and any installed
  plugins exist offline: include the packed `overlord-cli` tarball/dir and
