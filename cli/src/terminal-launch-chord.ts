...
const MODIFIER_ALIASES: Record<string, ParsedTerminalChord['modifiers'][number]> = {
  cmd: 'command',
  command: 'command',
  meta: 'command',
  ctrl: 'control',
...
export function parseTerminalLaunchChord(input: string): ParsedTerminalChord | null {
  ...
  const modifiers: ParsedTerminalChord['modifiers'] = [];
...
export function formatTerminalLaunchChord(chord: ParsedTerminalChord): string {
...
export function appleScriptKeystrokeClause(chord: ParsedTerminalChord): string {
  const modifierClause =
    chord.modifiers.length > 0
      ? ` using {${chord.modifiers.map(modifier => `${modifier} down`).join(', ')}}`
      : '';

  const keyLiteral =
    chord.key.length === 1 && chord.key !== '\\'
...
export function normalizeTerminalLaunchPlacement(
