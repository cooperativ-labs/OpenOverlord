...
<Aside type="danger">
**Attach first, deliver last.** Attaching is itself the go-ahead to start work —
do not attach and then wait for further confirmation. Only `connect` and
`load-context` are for inspection without acting.
</Aside>
...
<Aside type="tip">
Discussing or opening a mission should **submit** its draft objective
(`ovld protocol discuss-objective`), signalling active discussion. Only an
explicit "do this" / "execute it" should make you **attach** and start work.
</Aside>
...
<Aside type="tip">
The `--summary` is the first thing the reviewer reads. Write it as a short
narrative — what you did and what's next — not a command list.
</Aside>
...
<Aside type="danger">
Never revert, restore, or delete another agent's work to make delivery succeed —
no `git checkout`/`restore`/`reset` of edits you didn't make, no deleting
unfamiliar files, no `ovld protocol revert` on another objective's checkpoint.
</Aside>
